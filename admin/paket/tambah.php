...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_paket)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO paket (nama_paket, harga_per_kg, jenis) VALUES (?, ?, ?)");
        if ($stmt->execute([$nama_paket, $harga_per_kg, $jenis])) {
...
<head>
    ...
    <title>Tambah Paket Laundry - Admin</title>
...
<div class="main-content">
    <h4><i class="bi bi-plus-circle"></i> Tambah Paket Laundry Baru</h4>
        ...
        <div class="card-body">
            <form method="POST">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nama_paket" name="nama_paket" value="<?= htmlspecialchars($_POST['nama_paket'] ?? '') ?>" required>
                </div>
                <div class="mb-3">
                    <label for="harga_per_kg" class="form-label">Harga Per Kg/Satuan (Rp)</label>
                    <input type="number" class="form-control" id="harga_per_kg" name="harga_per_kg" value="<?= htmlspecialchars($_POST['harga_per_kg'] ?? '') ?>" min="0" step="100" required>
                </div>
                <div class="mb-3">
                    <label for="jenis" class="form-label">Jenis Paket</label>
                    <select class="form-select" id="jenis" name="jenis" required>
                        <option value="">-- Pilih Jenis --</option>
                        <option value="kg" <?= (($_POST['jenis'] ?? '') === 'kg') ? 'selected' : '' ?>>Per Kg</option>
                        <option value="satuan" <?= (($_POST['jenis'] ?? '') === 'satuan') ? 'selected' : '' ?>>Per Satuan</option>
                    </select>
                </div>
